...
## Documented flags omitted from normal help

The official CLI reference also documents `--max-turns N`, `--system-prompt-file PATH`, `--append-system-prompt-file PATH`, and `--permission-prompt-tool TOOL`. The first two were accepted together in a local parser probe (it reached the missing prompt-file error). Check the current official reference before using other hidden flags; they are not certified by the captured help. `--permission-prompt-tool` requires an actual permission host/MCP tool; use `--permission-prompts none` for ordinary unattended work.

Do not infer that all root flags compose: `--bg` and `-p` are mutually exclusive; output/input/budget/persistence flags marked print-only do not apply to background mode. Bare-mode context discovery wording differs between online documentation and local help; use normal mode when preserving user skills/auth is the goal.
